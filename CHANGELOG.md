...
- **독립 실행 파일 및 런처 구축**: 윈도우 원클릭 단독 실행 파일(`korean-law-web.exe`), 무음 실행 VBScript (`korean-law-web.vbs`), 대화형 CLI/웹 배치 런처(`run_interactive.bat`, `run_web.bat`, `run_web.ps1`) 리팩토링 및 CJS/ESM 호환 서빙(`findPublicPath()`) 반영.

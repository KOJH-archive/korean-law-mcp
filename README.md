...
## v4.8.0 — 원클릭 실행 런처 & 독립 실행 파일 (`.exe` / `.vbs`)

- **독립 실행 파일 (`korean-law-web.exe`)**: Node.js 환경이 설치되어 있지 않더라도 더블 클릭만 하면 백그라운드 웹 서버를 구동하고 브라우저(`http://localhost:3000`)를 즉시 자동으로 열어줍니다.
- **무음 런처 (`korean-law-web.vbs`)**: 검은색 CMD 콘솔 창 없이 백그라운드에서 깨끗하게 웹 서비스를 구동합니다.
- **대화형 CLI & 웹 런처 리팩토링**: `run_interactive.bat`, `run_web.bat`, `run_web.ps1`의 자동 컴파일 및 예외/콘솔 닫힘 보완.
